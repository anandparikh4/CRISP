...
/*
    ...
    Grant Member vote to Org2 user
    Join as Org2 user
*/
...
// await test_one_time_ops()

/*
    Write Private Record "record"
    
*/

async function test_one_time_ops(){
    await Login(worker.org.toLowerCase() , worker.org+"MSP" , worker.username)
    let response
    ...
    let paramsRTBFprivate = {
        Recordname: "record",
        sourceUsername: "Ellie",
        sourceOrgMSP: "Org1MSP"
    }

    let paramsRTBFprotected = {
        CommitteeName: "com",
        sourceUsername: "Ellie",
        sourceOrgMSP: "Org1MSP"
    }

    let c_acl_time_mean=0, c_acl_time
    let d_acl_time_mean=0, d_acl_time
    let a_com_time_mean=0, a_com_time
    let d_com_time_mean=0, d_com_time
    let rtbf_private_time
    let rtbf_protected_time_mean=0, rtbf_protected_time

    for(let i=0;i<10;i++){
        c_acl_time = await Construct_Access_Control_List(contract , paramsACL , true)
        await new Promise(resolve => setTimeout(resolve, 2000))     // sleep(2)
        d_acl_time = await Destruct_Access_Control_List(contract, paramsACL, true)
        await new Promise(resolve => setTimeout(resolve, 2000))     // sleep(2)

        a_com_time = await Assemble_Committee(contract, paramsCOM, true)
        await new Promise(resolve => setTimeout(resolve, 2000))     // sleep(2)
        rtbf_protected_time = await Vote_Protected_RTBF(contract, paramsRTBFprotected, true)
        await new Promise(resolve => setTimeout(resolve, 2000))     // sleep(2)
        d_com_time = await Disassemble_Committee(contract, paramsCOM, true)
        await new Promise(resolve => setTimeout(resolve, 2000))     // sleep(2)

        if(i > 0){
            c_acl_time_mean += c_acl_time
            d_acl_time_mean += d_acl_time
            a_com_time_mean += a_com_time
            d_com_time_mean += d_com_time
            rtbf_protected_time_mean += rtbf_protected_time
        }
    }
    ...
    a_com_time_mean /= 9
    d_com_time_mean /= 9
    rtbf_protected_time_mean /= 9

    console.log("Construct ACL time: ", c_acl_time_mean)
    console.log("Destruct ACL time: ", d_acl_time_mean)
    console.log("Assemble Committee time: ", a_com_time_mean)
    console.log("Disassemble Committee time: ", d_com_time_mean)
    console.log("Vote Protected Time: ", rtbf_protected_time_mean)

    rtbf_private_time = await Private_RTBF(contract, paramsRTBFprivate, true)
    await new Promise(resolve => setTimeout(resolve, 2000))     // sleep(2)
    console.log("Exercise Private RTBF time: ", rtbf_private_time)
}

// await test_join_leave()

/*
    Assemble committee with Dina. Grant vote to Ellie for all 3 access types
*/

async function test_join_leave(){
    await Login(worker.org.toLowerCase() , worker.org+"MSP" , worker.username)
    let response
    ...
    let j_m_times = [], j_m_time
    let j_r_times = [], j_r_time
    let j_w_times = [], j_w_time
    let l_m_times = [], l_m_time
    let l_r_times = [], l_r_time
    let l_w_times = [], l_w_time

    for(let i=0;i<10;i++){

        params["Type"] = "m"
        j_m_time = await Join_Committee(contract, params, true)
        await new Promise(resolve => setTimeout(resolve, 750))     // sleep(0.75)
        l_m_time = await Leave_Committee(contract, params, true)
        await new Promise(resolve => setTimeout(resolve, 750))     // sleep(0.75)

        params["Type"] = "r"
        j_r_time = await Join_Committee(contract, params, true)
        await new Promise(resolve => setTimeout(resolve, 750))     // sleep(0.75)
        l_r_time = await Leave_Committee(contract, params, true)
        await new Promise(resolve => setTimeout(resolve, 750))     // sleep(0.75)

        params["Type"] = "w"
        j_w_time = await Join_Committee(contract, params, true)
        await new Promise(resolve => setTimeout(resolve, 750))     // sleep(0.75)
        l_w_time = await Leave_Committee(contract, params, true)
        await new Promise(resolve => setTimeout(resolve, 750))     // sleep(0.75)

        if(i>0){
            j_m_times.push(j_m_time)
            l_m_times.push(l_m_time)
            j_r_times.push(j_r_time)
            l_r_times.push(l_r_time)
            j_w_times.push(j_w_time)
            l_w_times.push(l_w_time)
        }
    }

    console.log("mean_join_r: ", mean(j_r_times))
    console.log("std_join_r ", std(j_r_times))
    console.log("mean_join_w: ", mean(j_w_times))
    console.log("std_join_w ", std(j_w_times))
    console.log("mean_join_m: ", mean(j_m_times))
    console.log("std_join_m ", std(j_m_times))
    console.log("mean_leave_r: ", mean(l_r_times))
    console.log("std_leave_r ", std(l_r_times))
    console.log("mean_leave_w: ", mean(l_w_times))
    console.log("std_leave_w ", std(l_w_times))
    console.log("mean_leave_m: ", mean(l_m_times))
    console.log("std_leave_m ", std(l_m_times))

    console.log("test_join_leave done")
}
