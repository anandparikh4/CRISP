...
export async function Construct_Access_Control_List(contract , params , log = false){
    let t1 = Date.now()
    const args = {
    ...
    let t2 = Date.now()
    if(log){
        console.log("Construct ACL time: " , t2-t1)
        return t2-t1
    }
    return response
}

export async function Destruct_Access_Control_List(contract , params , log = false){
    let t1 = Date.now()
    const args = {
        Recordname: params["Recordname"]
    }
    ...
    let t2 = Date.now()
    if(log){
        console.log("Destruct ACL time: " , t2-t1)
        return t2-t1
    }
    return response
}
